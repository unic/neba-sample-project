...
public class PermissionSetup {
    ...
    private void grantWritePermissionsBelowInboxNodeToAnonymous(Session session, JackrabbitAccessControlManager acMgr) throws RepositoryException {
        ...
        deny(session.getNode(Constants.CONTACT_INBOX).getParent(), principal.getName(), JCR_READ);
        ...
        JackrabbitAccessControlList list = (JackrabbitAccessControlList) acMgr.getApplicablePolicies(Constants.CONTACT_INBOX).nextAccessControlPolicy();
        ...
        acMgr.setPolicy(Constants.CONTACT_INBOX, list);
    }

    private void removePreviousInboxACLs(JackrabbitAccessControlManager acMgr) throws RepositoryException {
        AccessControlPolicyIterator applicablePolicies = acMgr.getApplicablePolicies(Constants.CONTACT_INBOX);

        if (!applicablePolicies.hasNext()) {
            for (AccessControlPolicy policy : acMgr.getPolicies(Constants.CONTACT_INBOX)) {
                acMgr.removePolicy(Constants.CONTACT_INBOX, policy);
            }
        }
    }
}
